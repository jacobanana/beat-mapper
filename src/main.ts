...
app.bus.on('step', () => setTimeout(() => renderer.resize(), 0));
...
matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => renderer.recolor());
new MutationObserver(() => renderer.recolor()).observe(document.documentElement, { attributes: true, attributeFilter: ['data-theme'] });
